...
//parseURL takes a url generated by httptest.Server and returns a Config
func parseURL(url string) Config {
	urlParts := strings.Split(url[7:], ":")
	ip := urlParts[0]
	port, _ := strconv.Atoi(urlParts[1])

	config := Config{ip, port}
...
func TestNewPeer(t *testing.T) {
	...
	p := NewPeer(config)
	if p.Config != config {
		t.Errorf("p.Config == %v; want %v", p.Config, config)
	}
...
func TestPeerGet(t *testing.T) {
	...
	for _, testCase := range testCases {
		func() {
			...
			p.Config = parseURL(testServer.URL)
...
func TestPeerGetPeers(t *testing.T) {
	testCases := []PeersResponse{
		{Peers: nil},
		{Peers: []Config{Config{"127.0.0.1", 8080}}},
		{Peers: []Config{Config{"127.0.0.1", 8080}, Config{"127.0.0.1", 8081}}},
	}
	...
	for _, testCase := range testCases {
		func() {
			...
			if len(configs) != len(testCase.Peers) {
				t.Errorf("p.GetPeers() == %v; want %v", configs, testCase.Peers)
			}
		}()
	}
}
...
func TestPeerGetPeersFail(t *testing.T) {
	...
	if configs != nil {
		t.Errorf("p.GetPeers() == %v; want %v", configs, nil)
	}
}
